import { Meta } from '@storybook/blocks'

<Meta title="Docs/Tutorials/Themes" />

# Themes

> This version relies on <b>Carbon v11</b>. If you're using Carbon v10,
>
> <a href="https://carbon-charts-0x.netlify.app" target="_blank" rel="noreferrer">
> 	see the legacy demo site
> </a>
> .

We support all 4 Carbon themes (white, g10, g90 & g100), which are all included inside the main CSS
bundle. After importing the carbon styles, pass in one of the four Carbon themes in options like in
the examples below.

**If using CSS:**

You'd need to import our **CSS bundle** `@carbon/charts/styles.css`

e.g.

```html
<div id="my-sample-chart"></div>
```

```ts
const chartHolder = document.getElementById('my-sample-chart')
new SimpleBarChart(chartHolder, {
	data,
	options: {
		...myChartOptions,
		theme: 'g100' // Pass theme in options
	}
})
```

**If using SCSS:**

```scss
@import '@carbon/charts/styles/styles.scss';
```

```ts
const chartHolder = document.getElementById('my-sample-chart')
new SimpleBarChart(chartHolder, {
	data,
	options: {
		...myChartOptions,
		theme: 'g100' // Pass theme in options
	}
})
```

<br />
## Color palette - Our color palette udpates automatically on theme switches
