...
<p>
	Carbon Charts uses a tabular data format. The package exports a TypeScript type called <strong
		>ChartTabularData</strong
	>. This format accomodates all types of charts. Each element in the array is a datapoint.
</p>

<CodeSnippet
	type="multi"
	code={`const simpleBarData = [
  { key: 'Qty', value: 65000 },
...
<CodeSnippet
	type="multi"
	code={`export const bubbleDoubleLinearOptions = {
  title: "Bubble (linear)",
  axes: {
    bottom: {
...
<CodeSnippet
	type="multi"
	code={`const myChart = new PieChart({
  data: ...,
  options: ...
})
