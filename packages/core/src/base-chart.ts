...
export class BaseChart {
		...
		const resizeObserver = new ResizeObserver((entries, observer) => {
			for (const entry of entries) {
				if (Math.abs(containerWidth - this.holder.clientWidth) > 1
					...
					selectAll(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip").style("display", "none");
	...
	addLegend() {
		if (this.container.select(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip").nodes().length > 0) {
			return;
		}
	...
	positionLegend() {
		if (this.container.select(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip")
				.nodes().length > 0
			&& this.container.select(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip")
	...
	addTooltipOpenButtonToLegend() {
		...
		thisLegend.append("div")
			.attr("class", this.options.rtl ? "expand-btn-rtl" : "expand-btn")
	...
	openLegendTooltip(target) {
		selectAll(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip").remove();
		const mouseXPoint = mouse(this.container.node())[0];
		const windowXPoint = event.x;
		let tooltip;
		if (this.container.select(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip").nodes().length > 0) {
			tooltip = selectAll(this.options.rtl ?  ".legend-tooltip-rtl" : ".legend-tooltip").style("display", "block");
			tooltip.select("arrow").remove();
		} else {
			tooltip = this.container.append("div")
				.attr("class", this.options.rtl ? "tooltip chart-tooltip legend-tooltip-rtl" : "tooltip chart-tooltip legend-tooltip")
				.style("display", "block")
