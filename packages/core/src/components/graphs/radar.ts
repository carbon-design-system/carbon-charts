...
export class Radar extends Component {
	...
	render(animate = true) {
		...
		const yTicks = computeTicks(completeYTicks); // ticks starting from not 0
...
// Given an array of ticks generated by d3, returns another of ticks
// uniformly spaced (like the original d3 ticks array) but with the
// first tick computed as the nearest MIN_DEFAULT_TICK multiple of step.
// Example:
// 		yTicks = [0, 5, 10, 15, 20, 25]
// 		MIN_DEFAULT_TICK = 8 -> minRoundedTick = 10
// 		-> [10, 15, 20, 25]
function computeTicks(yTicks: number[]) {
	const stepTicks = yTicks[1] - yTicks[0];
	const MIN_DEFAULT_TICK = 8;
	const minRoundedTick = roundXToN(MIN_DEFAULT_TICK, stepTicks);
	return range(minRoundedTick, max(yTicks), stepTicks);
}
