...
export class Radar extends Component {
    ...
    render(animate = true) {
        ...
        // blobs
        const blobsGroup = DOMUtils.appendOrSelect(svg, 'g.blobs').attr(
            'role',
            Roles.GROUP
        );

        const blobGroups = blobsGroup
            .selectAll('g.blob')
            .data(this.groupedDataNormalized, (group) => group.name);

        const blobGroupsEnter = blobGroups
            .enter()
            .append('g')
            .attr('class', 'blob')
            .style('transform-origin', '25% 50%')
            .attr('role', Roles.GROUP)
            .attr('data-name', 'blob');

        const blobsEnter = blobGroupsEnter.append('path').attr('class', 'blob');
        const blobStrokesEnter = blobGroupsEnter
            .append('path')
            .attr('class', 'blob stroke');
        blobsEnter
            .merge(blobStrokesEnter)
            .attr('class', (group) =>
        ...
        const blobGroupsExit = blobGroups.exit();

        blobGroupsExit
            .attr('opacity', 0)
            .remove();

        const blobs = blobGroups.merge(blobGroupsEnter).selectAll('path.blob');
        // .each(function () {
        //  console.log('Data', select(this).datum());
        // });
