...
const Link = ({link}) => {
  ...
  return (
    <Edge path={path.toString()} />
  );
};

const Node = ({x, y}) => {
  return (
      <foreignObject transform={`translate(${x},${y})`} height={48} width={48} style={{ overflow: "visible" }}>
...
const Elk = ({nodes, links, layout}) => {
  ...
  return (
    <div className={`force`} style={{ position: `relative`, height: 1000, width: 1000, margin: 32 }}>
